...
const createMonthYear = isoDate => {
  ...
  // return `${month} '${year}`; //January '24, this is for label
  //`${month.toLowerCase()}${year}`;  //this is for name
};
...
//For each element in tempData.started_at
//pass as argument to createMonthYear
//For each element in tempData.conclusion
//if 'cancelled' or 'failure'
//increase failure by 1 and total by 1
//if 'success'
//increase success by 1 to total by 1

//To calculate workflow run time
//For each element in tempData
//Get started_at value of tempData.steps[0]
//Get completed_at value of tempData.steps[length - 1]
//Use as arguments to timeDifSeconds and push result to metrics.monthData.runTimes (need to lookup using getMonthDate)
...
const reformatData = array => {
  ...
  metricsObj.monthData = monthDataArr;
};
